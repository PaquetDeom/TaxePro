...
public class TaFieldGroupFieldFactory extends DefaultFieldGroupFieldFactory {
	...
	public <T extends Field> T createField(Class<?> type, Class<T> fieldType) {
		...
		} else if (TaItem.class.isAssignableFrom(type)) {
	...
	protected TwinColSelect createListBuilderItem(Class type) {
		try {
			...
			Collection<TaItem> values = new TaContainer(item).findAll();
	...
	protected ComboBox createComboItem(Class<?> type) {
		Collection<TaItem> values = new TaContainer(type).findAll();
